void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	grain *= mix(0.9, 1.0,
		     sin(iTime * 0.5 + uv.x * 50.0)); // Slow, animated grain
	...
	float scratch =
		scratchChance * scratchIntensity * step(0.9, fract(sin(iTime * 20.0 + uv.x * 500.0) * 43758.545));
	...
	blotch *= smoothstep(0.0, 0.1,
			     fract(sin(dot(uv * 20.0, vec2(15.45, 25.33))) * 546.89)); // Irregular, fading blotches
	vec3 blotchEffect = mix(vec3(1.0), vec3(0.4, 0.3, 0.3),
				blotch * 0.8); // Dark reddish blotches
