...
const float bayerPattern[4] = float[4](0x0514, // Encoding 0,8,2,10
...
{
	int idx = (x & 3) + ((y & 3) << 2);
	return float((int(bayerPattern[y & 3]) >> ((x & 3) << 2)) & 0xF) *
	       (1.0 / 16.0);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	float threshold =
		getBayerFromPacked(int(fragCoord.x), int(fragCoord.y));
