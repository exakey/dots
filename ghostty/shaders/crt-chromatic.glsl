...
vec4 toRgb(vec4 oklab)
{
	...
	float m = m_ * m_ * m_;
	float s = s_ * s_ * s_;
	vec3 linear_srgb = vec3(4.0767416621 * l - 3.3077115913 * m + 0.2309699292 * s,
				-1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * s,
				-0.0041960863 * l - 0.7034186147 * m + 1.7076147010 * s);
	return vec4(clamp(f(linear_srgb.r), 0.0, 1.0), clamp(f(linear_srgb.g), 0.0, 1.0),
...
// Bloom sample points
const vec3[24] samples = { vec3(0.1693761725038636, 0.9855514761735895, 1),
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	float mask = 1.0 - (MASK_INTENSITY * (0.5 + 0.5 * sin(mask_pos.x * 3.14159) * sin(mask_pos.y * 3.14159)));
