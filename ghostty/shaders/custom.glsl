...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	float glitchAmount =
		SS(DURATION * .001, DURATION * AMT, mod(t, DURATION));
	...
	distortion *= gnoise(vec3(0., y * .02, t * 250.)) *
		      (glitchAmount * 2. + .025);
	...
	col += (.15 + .65 * glitchAmount) *
	       (hash33(vec3(fragCoord, mod(float(iFrame), 1000.))).r) *
	       displayNoise;
	col -= (.25 + .75 * glitchAmount) *
	       (sin(4. * t + uv.y * iResolution.y * 1.75)) * displayNoise;
